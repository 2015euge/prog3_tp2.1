...
class Reservation {
    ...
    get info() {
        const formattedDate = this.date.toLocaleString(); 
        return `Reserva #${this.id} - ${formattedDate} | Cliente: ${this.customer.info} | Comensales: ${this.guests}`;
    }

    static validateReservation(reservationInfo) {
        const currentDate = new Date();
        const reservationDate = new Date(reservationInfo.date);

        if (reservationDate < currentDate || reservationInfo.guests <= 0) {
            return false;
        }
